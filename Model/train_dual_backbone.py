...
def main():
    ...
    parser.add_argument("--mace-pretrained", default=None,
                        help="Path to pre-trained MACE checkpoint")
    parser.add_argument("--alignn-checkpoint-dir", default=None,
                        help="Optional dir with ALIGNN fold{N}_best.pt checkpoints")
    ...
    if args.alignn_checkpoint_dir:
        ...
        alignn_model = load_alignn_model(
            alignn_ckpt, device, dropout=args.dropout)
    else:
        print("Initializing ALIGNN; the full DBGFN checkpoint will supply its weights.")
        alignn_model = ALIGNNMultiTaskPyG(
            atom_input_dim=94, edge_input_dim=80, angle_input_dim=40,
            hidden_dim=256, n_alignn_layers=4, n_gcn_layers=4,
            n_gap_classes=2, n_eh_classes=2, dropout=args.dropout,
        )
