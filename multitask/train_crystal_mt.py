"""
Unified multi-task training script for ALIGNN and M3GNet on chalcohalide data.

Usage:
  python train_crystal_mt.py <data_dir> --model alignn --fold 0
  python train_crystal_mt.py <data_dir> --model m3gnet  --fold 0
"""
...
def main():
    ...
    parser.add_argument("--model", default="alignn", choices=["alignn", "m3gnet"])
    ...
    # Architecture
    parser.add_argument("--hidden-dim", type=int, default=None,  # auto by model
                        help="256 for ALIGNN, 64 for M3GNet (defaults)")
    ...
    # Default hidden dim per model
    if args.hidden_dim is None:
        args.hidden_dim = 256 if args.model == "alignn" else 64

    # Default checkpoint dir
    if args.checkpoint_dir is None:
        args.checkpoint_dir = f"checkpoints/{args.model}_mt"
    ...
    print(f"Model: {args.model} | fold={args.fold} | hidden={args.hidden_dim}")
    ...
    # ── Model ────────────────────────────────────────────────────────
    n_edges = dataset[0].edge_attr.size(1) if hasattr(dataset[0], 'edge_attr') else args.dist_bins
