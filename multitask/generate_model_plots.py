#!/usr/bin/env python3
"""
Generate OOF predictions and 3 plot types per model:
  1. Bandgap scatter (predicted vs. true)
  2. GT classification: Acc + F1 per fold bar chart
  3. EH classification: Acc + F1 per fold bar chart
"""
...
# ──────────────────────────────────────────────
# Inline Normalizer (same logic as training scripts)
# ──────────────────────────────────────────────
class Normalizer:
    def __init__(self):
        self.mean = 0.0
...
# ──────────────────────────────────────────────
# 1. CGCNN / ALIGNN / M3GNet — from OOF CSV
# ──────────────────────────────────────────────
oof_df = pd.read_csv(f"{PROJECT}/six_model_oof_predictions.csv")

def get_from_oof(name):
    m = oof_df[oof_df['model'] == name]
    return dict(bg_true=m['bg_true'].values.astype(float),
                bg_pred=m['bg_pred'].values.astype(float),
                gt_true=m['gt_true'].values.astype(int),
                gt_pred=m['gt_pred'].values.astype(int),
                eh_true=m['eh_true'].values.astype(int),
                eh_pred=m['eh_pred'].values.astype(int),
                fold=m['fold'].values.astype(int))

for name in ['CGCNN', 'ALIGNN', 'M3GNet']:
    make_all_plots(name, get_from_oof(name))


# ──────────────────────────────────────────────
# Shared: build MACE foundation model once
# ──────────────────────────────────────────────
...
def run_mace():
    ...
    for fold in range(5):
        ckpt = torch.load(f"{CKPT_BASE}/mace_mt/fold{fold}_best.pt",
        ...
        loader = PygLoader([dataset[i] for i in test_idx], batch_size=64, shuffle=False)
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                bg_p, gt_l, eh_l = model(batch.to_dict())
...
print("\n=== MACE ===")
make_all_plots('MACE', run_mace())


# ──────────────────────────────────────────────
# 3. MACE+ALIGNN (dual_backbone)
# ──────────────────────────────────────────────
def run_mace_alignn():
    ...
    for fold in range(5):
        ...
        dataset = DualBackboneDataset(
            args['mace_cache_dir'], args['crystal_cache_dir'], args['data_dir'],
            merge_metal_indirect=args['merge_metal_indirect'],
            log_bg=args['log_bg'],
        )
        _, _, test_idx = stratified_kfold_split(
