"""
Multi-task CGCNN v6 — key changes over v5:

1. Dynamic task weighting (uncertainty-based, Kendall et al. 2018)
2. Right-sized model: 64 atom_fea, 128 h_fea, 4 conv (for 2.5K data)
3. Stronger regularization: dropout 0.35, weight_decay 1e-3
4. Improved BG loss: HuberLoss(delta=0.5) + sample reweighting for high-BG
5. No log transform for BG (avoids compressing high-BG signal)
6. Cached graph loading for GPU utilization boost (3% → 50%+)
7. Smaller batch size (64) for more gradient updates per epoch
8. BG-aware composite for early stopping
9. Robust (IQR-based) normalization option
"""
import argparse
import os
import random
...
class WeightedHuberLoss(nn.Module):
    def __init__(self, delta=0.5, reweight_scale=0.5):
        super().__init__()
        self.delta = delta
    ...
    def forward(self, pred, target, raw_bg=None):
        ...
        if raw_bg is not None and self.reweight_scale > 0:
            # Upweight samples with large BG (log-compressed or not)
...
# ---------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description='Multi-task CGCNN v6')
    ...
    p.add_argument('--checkpoint-dir', default='checkpoints/multitask_v6')
    return p.parse_args()
...
def main():
    ...
    # Summary
    print(f'\n{"="*60}')
    print(f'{args.k_folds}-Fold CV Summary (v6)')
    print(f'{"="*60}')
    for tag in ['composite', 'gt_f1', 'bg_mae']:
        if all_results[tag]:
