#!/usr/bin/env python3
"""
predict_jacs_eh01.py — MACE+ALIGNN DualBackbone inference (eh01 retrained models)
...
CKPT_DIR   = os.path.join(PROJECT, "checkpoints/dual_backbone_eh01")
ALIGNN_DIR = os.path.join(PROJECT, "checkpoints/alignn_mt_eh01")
...
def main():
    ...
    parser.add_argument("output_csv", nargs="?", default="predictions_jacs_eh01.csv")
    ...
    print(f"Loading {N_FOLDS} fold checkpoints (eh01) …")
    models, norms = [], []
    ...
    for i, fpath in enumerate(struct_files, 1):
        ...
        for fold_idx, (model, (nm, ns)) in enumerate(zip(models, norms)):
            try:
                ...
                import traceback; traceback.print_exc(); print(f"  [PRED ERROR] {name} fold {fold_idx}: {e}")
