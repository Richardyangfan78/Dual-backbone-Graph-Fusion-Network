#!/usr/bin/env python3
"""Build the augmented strict ABC prediction set.

Expected set = strict Type1/Type2/Type3 grid plus selected training-template
supplement patterns generated under the same A/B/C/Ch/X role constraints.
"""
...
FINAL_DIR = os.path.join(BASE, "Data/predict_final_strict_abc_augmented")
MANIFEST = os.path.join(BASE, "Data/predict_final_strict_abc_augmented_manifest.csv")
MISSING_CSV = os.path.join(BASE, "Data/predict_final_strict_abc_augmented_missing.csv")
...
def enumerate_pattern(pattern: str):
    ...
    for elements in product(*choices):
        ...
        for (role, _), element in zip(slots, elements):
            by_role[role].append(element)
        if any(len(values) != len(set(values)) for values in by_role.values()):
            continue
        for ch in CH:
            for x in X:
...
def expected_formulas() -> dict[str, str]:
    ...
    for a in A:
        for c in C:
            ...
            for ch in CH:
                for x in X:
                    expected[reduced_formula_from_counts({a: 1, c: 1, ch: 1, x: 2})] = "Type1_A1_C1_Ch1_X2"

    for c in C:
        for ch in CH:
            for x in X:
                expected[reduced_formula_from_counts({c: 1, ch: 1, x: 1})] = "Type2_C1_Ch1_X1"
    ...
    for b in B:
        for c in C:
            ...
            for ch in CH:
                for x in X:
                    expected[reduced_formula_from_counts({b: 2, c: 1, ch: 2, x: 3})] = "Type3_B2_C1_Ch2_X3"

    for pattern in SUPPLEMENT_PATTERNS:
        for formula in enumerate_pattern(pattern):
            expected[formula] = "TrainTemplate_" + pattern
...
def reset_final_dir():
    ...
    if not final_abs.startswith(data_dir + os.sep):
        raise RuntimeError(f"Refusing to reset unexpected directory: {FINAL_DIR}")
    if os.path.basename(final_abs) != "predict_final_strict_abc_augmented":
        raise RuntimeError(f"Refusing to reset unexpected directory: {FINAL_DIR}")
...
def main() -> int:
    ...
    print("Augmented strict ABC final set build complete")
    ...
    print("Copied by type:", dict(by_type))
