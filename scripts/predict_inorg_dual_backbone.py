...
def load_fold_model(fold, mace_model_base, device, ckpt_dir, alignn_dir):
...
def main():
    ...
    parser.add_argument("--alignn-checkpoint-dir", default=None,
                        help="Directory with ALIGNN fold{N}_best.pt files")
    ...
    ckpt_dir = Path(args.checkpoint_dir or
                    project / "checkpoints" / "dual_backbone_inorg").resolve()
    alignn_dir = Path(args.alignn_checkpoint_dir or
                      project / "checkpoints" / "alignn_mt_inorg").resolve()
    sys.path.insert(0, str(project / "multitask"))
